...
function SliderComponent({ movies, handleSelectChange }) {
    ...
    return (
        <>
            <section className="w-full mt-2 mx-auto max-w-[1200px] 2xl:max-w-[56rem] overflow-hidden text-white rounded-lg slider" id="slider">
                {Array.isArray(moviesAll) ? moviesAll.map((movie, index) => {
                    const imageUrl = image(movie);
                    return imageUrl ? (
                        <figure key={index} className="relative w-full h-full aspect-[5/2] slider-childs" data-id-slider={index}>
